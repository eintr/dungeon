...
int proxy_context_put_runqueue(proxy_context_t *my)
{
	...
	while ((ret = llist_append_nb(my->pool->run_queue, my))) {
		mylog(L_ERR, "ret is %d, put context to run queue failed, %p\n", ret, my);
	}
	mylog(L_DEBUG, "put context to run queue success, %p\n", my);
	return 0;
}
...
int proxy_context_put_epollfd(proxy_context_t *my)
{
	...
	if (my->state != STATE_READHEADER) {
		...
		if (ret == -1) {
			mylog(L_ERR, "mod my->epoll_context failed: %s\n", strerror(errno));
		}
	}
	return 0;
}
...
static int proxy_context_driver_accept(proxy_context_t *my)
{
	...
	while (1) {
		...
		if (my->pool->nr_idle + my->pool->nr_busy +1 > my->pool->nr_total) {
			...
			mylog(L_DEBUG, "new proxy is created, it is  %p\n", newproxy);
...
static int proxy_context_driver_readheader(proxy_context_t *my)
{
	...
	for (pos = 0; pos<HEADERSIZE; ) {
		...
		if (len<0) {
			...
			mylog(L_ERR, "read header failed, errno=%d\n", -len);
...
int proxy_context_driver_iowait(proxy_context_t *my)
{
	...
	if (nfds <= 0) {
		mylog(L_DEBUG, "in driver epoll_wait return %d\n", nfds);
		//return 0;
		goto putfd;
	}
	for (i = 0; i < nfds; i++) {
	...
	if (io & EPOLLIN) {
		...
		while (my->c2s_wactive) {
			...
			if (res == 0) {
				/* client close */
				mylog(L_DEBUG, "client is closed, close all connection, STATE_CLOSE, %p\n", my);
...
{
	mylog(L_DEBUG, "begin driver close, %p\n", my);
...
int proxy_context_driver_rejectclient(proxy_context_t *my)
{
	...
	mylog(L_ERR, "begin driver rejectclient, %p\n", my);
...
int proxy_context_driver_term(proxy_context_t *my)
{
	...
	mylog(L_DEBUG, "context driver terminate, %p\n", my);
